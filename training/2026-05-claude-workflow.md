...
- **Set your plan and its data-handling defaults intentionally.** The privacy-relevant differences between individual and team-tier plans are mostly about *defaults* — training-on-your-data, retention windows, isolated workspace where applicable — not whether the guarantees can be achieved at all. Audit the settings on whatever tier you're on; the right defaults are at least as important as the tier name. **What no cloud plan substitutes for is the local LLM (§7):** cloud still ships data to a vendor's servers — it just shapes what they do with it. Local-only is the only answer for code that genuinely cannot leave the building.
