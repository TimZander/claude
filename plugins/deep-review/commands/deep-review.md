...
It prints `KEY=value` lines on stdout. `HOST`, `KIND`, `CURRENT_BRANCH` and `IN_WORKTREE` are always present. `REF_ID` appears whenever a reference was found, and `SOURCE_BRANCH`, `TARGET_BRANCH`, `STATE` and `BRANCH_MATCH` appear only when `KIND=pr`. Errors go to stderr, so stdout is never anything but `KEY=value` lines.
