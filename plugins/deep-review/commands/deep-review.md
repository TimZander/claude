...
- **Is entity-keyed config complete?** For JSON/YAML config files keyed by an entity (site ID, resort, environment, etc.), verify that every expected entity is represented. If any are missing, flag it and ask whether the omission is intentional — a config that covers 2 of 3 active sites with no documented reason is a defect, not a partial success.
...
- **Silent UX bail-outs.** The user-facing analog of a swallowed exception: flag event handlers that return with no feedback when a precondition isn't met (e.g., `canCalculate()` returns false → the click handler does nothing). If clicking a button does nothing under certain conditions — no loading indicator, no validation message, no disabled state — the missing user feedback is a defect, not a design choice.
