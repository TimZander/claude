...
**This checks out the PR's branch.** When the resolved branch is not the one you have checked out, the review tells you and **asks before switching** — it detaches your working directory to `origin/<source-branch>` and restores your original ref afterwards. It refuses to start if your tree is dirty, and it never switches silently. Requires a clean tree, `gh` (GitHub) or `az` with the `azure-devops` extension (ADO).

Reading an ADO work item additionally needs **Python 3.7+** on `PATH`, as `python3`, `python`, or `py -3`. It renders the story out of the HTML ADO stores it in. Without it the review still runs and reports the work item as found-but-unfetchable. The reader takes the organization from `DEEP_REVIEW_ADO_ORG`; `/deep-review` sets that for you from the git remote, and a hosted runner is expected to export it alongside an allow-rule for `scripts/workitem-read.sh`. Fork PRs are not supported: their source branch does not exist on `origin`, so use `gh pr checkout <N>` and then `branch:<name>`.
