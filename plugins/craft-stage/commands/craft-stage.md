...
Output a ready-to-paste single-line `git add` command that stages only the meaningful files.

**Format:** `git add file1 file2 ...`

- List every meaningful file explicitly so junk files are excluded.
- The command must be a single line — no line breaks, no line continuations.
...
```powershell
git add src/Profile.cs src/Validator.cs tests/ProfileTests.cs
```

**Do NOT run the command.** Only output it for the user to copy.
